	...
	*/
	function getListings (options, html) {
		...
		$('div.content').find('p.row').each(function (i, element) {
            ...
            if (options.fullListing  &&  rurl) {
	          var idx = rurl.indexOf('://') + 3;
	          var idx1 = rurl.indexOf('/', idx);
	          var opt = { hostname: rurl.substring(idx, idx1), path: rurl.substring(idx1), fullListing: true }
  			  self.request.get(options, function (err, data) {
			 	if (err) 
				  return callback(err)
                listing.description = callback(null, getDescription(options, data))
			  })
	          // self.search(opt, '', function (err, l1) {
	          //   listing.description = l1.description
	          // })
            }
			// make sure lat / lon is valid
			if (typeof listing.coordinates.lat === 'undefined' ||
	...
	function getDescription(options, html) {
		...
		var description = $('section#postingbody').html().trim()
		return description
	}
	/*
