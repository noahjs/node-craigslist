var
	...
	*/
	function getRequestOptions (options, query) {
		...
		requestOptions.hostname = (options.city || self.options.city || '') + baseHost;
	...
	*/
	self.search = function (options, query, callback) {
		...
		self.request.get(options, function (err, data) {
			...
			return callback(null, options.isSites ? getSites(data) : (options.rss ? getRssListings(options, data) : getListings(options, data)))
		});
	};
